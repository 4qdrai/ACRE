...
| **Standard Transformer** | GPT-like / Llama-class | Autoregressive next-token prediction |
...
|----------|--------|----------|
| **ACRE** | ✅ **Excellent** | Compositionality is algebraic and provable (Theorem 5). The concept algebra operations (⊕, ⊗, ⊖, Π) are defined to preserve systematic structure. On the rigorous COGS benchmark, ACRE targets >90% accuracy on systematic generalization splits where standard seq2seq models plateau at ~15-35%, with initial validation on SCAN (99.2%). Novel compositions of known primitives are handled correctly by structural induction — the system doesn't need to memorize combinations. |
...
**Verdict:** ACRE provides the strongest compositionality guarantee — algebraic by design, proven by structural induction, and validated empirically on COGS and CFQ. Neuro-symbolic systems are the closest competitor but lack scalability.
...
| **Standard Transformer** | ❌ Very Poor | Training large-scale GPT-like models requires millions of GPU-hours and hundreds of millions of dollars (Epoch AI, 2024). Even smaller models (7B) require 1–2T tokens and thousands of GPU-hours. This is the fundamental scaling wall that ACRE aims to break. |
...
| **Standard Transformer** | ✅ Good | Modern autoregressive transformers (GPT-like architectures) handle multiple modalities through unified tokenization. However, the integration is at the token level — there's no structural alignment between modalities. Cross-modal reasoning relies on statistical co-occurrence, not formal structure. |
